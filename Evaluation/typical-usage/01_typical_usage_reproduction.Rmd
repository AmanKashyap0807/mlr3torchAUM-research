---
title: "Typical Usage Vignette Reproduction"
author: "Aman Kashyap"
date: "`r Sys.Date()`"
output: 
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Typical Usage Vignette Reproduction}
  %\VignetteEngine{knitr::knitr}
  %\VignetteEncoding{UTF-8}
---

```{r setup, include=FALSE}
# FORCE CPU execution for both main session and any parallel workers
...
This is part 1 of the GSoC 2026 easy test for mlr3torchAUM package. The objective is to reproduce the typical usage vignette demonstrating stratified batch sampling for imbalanced classification using the spam dataset.

Original vignette: https://github.com/tdhock/mlr3torchAUM/blob/main/vignettes/Typical_usage.Rmd

Execution date: `r Sys.Date()`

# 1. Prerequisites

This vignette assumes the following packages are already installed (including the torch C++ backend via `torch::install_torch()`):

```
# CRAN packages
install.packages(c("torch", "mlr3", "mlr3torch", "data.table", "ggplot2"))

# mlr3torchAUM from GitHub
remotes::install_github("tdhock/mlr3torchAUM")
```

Note: the install commands above are just for reference and I did not execute them during rendering. I read in Writing R Extensions (section 1.4 on vignettes) that `install.packages()` should not be called inside vignette chunks because CRAN runs them automatically during package checks. In an actual package vignette I would list torch and mlr3torch under Suggests in DESCRIPTION and use `requireNamespace()` guards instead.

# 2. Vignette Reproduction

Original source: https://github.com/tdhock/mlr3torchAUM/blob/main/vignettes/Typical_usage.Rmd

The sections below reproduce the typical usage vignette exactly as written by tdhock, with inline validation checks added to verify numerical correctness.
...
```{r validate-dataset}
stopifnot(count_tab["spam"] == 310)
stopifnot(count_tab["nonspam"] == 2480)
cat("Dataset: 310 spam, 2480 non-spam\n")
```
...
```{r validate-batch-sampler}
stopifnot(batch_size == 9)
stopifnot(nrow(batch_dt) == 310)
cat("Batch sampler: 310 batches of size 9\n")
```
...
```{r validate-batching}
if(torch_available){
  # Count problematic batches in random sampling
  zero_spam_count <- sum(batch_y_counts[set_batch_sampler==FALSE]$spam == 0)
  
  # Check stratified sampling always has spam
  min_spam_stratified <- min(batch_y_counts[set_batch_sampler==TRUE]$spam)
  
  cat("Comparison validated:\n")
  cat("  - Random sampling:", zero_spam_count, "batches with no spam\n")
  cat("  - Stratified sampling: min", min_spam_stratified, "spam per batch\n")
  
  stopifnot(zero_spam_count > 100)
  stopifnot(min_spam_stratified >= 1)
}
```
...
```{r validate-benchmark}
if(torch_available){
  # Check that benchmark ran successfully
  stopifnot(length(result_list) == 4)
  stopifnot(all(names(result_list) %in% c("random_full", "stratified_full", 
                                            "random_stoch", "stratified_stoch")))
  
  # Verify full batching methods produce similar results
  w1 <- result_list$random_full$weights
  w2 <- result_list$stratified_full$weights
  weight_diff <- max(abs(w1 - w2))
  
  cat("Benchmark results:\n")
  cat("  - All 4 sampling methods completed successfully\n")
  cat("  - Full batching weight difference:", weight_diff, "\n")
  cat("  - (Small difference confirms randomness control)\n")
  
  stopifnot(weight_diff < 1e-3)
}
```
...
```{r fig.width=12, fig.height=5, dpi=120, out.width="100%"}
if(torch_available){
